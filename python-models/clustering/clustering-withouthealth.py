...
KM2 = KMeans(n_clusters=2, init='k-means++', random_state=170)

KM2 = KM2.fit(dataset)

print("The cluster centroids are: \n", KM2.cluster_centers_)
print("Cluster", KM2.labels_)
print("Sum of distances of samples to their closest cluster center: ", KM2.inertia_)
...
def Popularity(y_data, y_name):
    plt.scatter(dataset.popularity, y_data, c=KM2.labels_, cmap=matplotlib.colors.ListedColormap(colors), s=75)
...
Popularity(dataset.tempo, 'tempo');
Popularity(dataset.popularity, 'popularity');
...
def Valence(y_data, y_name):
    plt.scatter(dataset.valence, y_data, c=KM2.labels_, cmap=matplotlib.colors.ListedColormap(colors), s=75)
    plt.xlabel('valence')
    plt.ylabel(y_name)
    plt.show()
Valence(dataset.energy, 'energy');
Valence(dataset.dance, 'dance');
Valence(dataset.tempo, 'tempo');
Valence(dataset.popularity, 'popularity');
# Valence(dataset.acoustic, 'acoustic');
# Valence(dataset.instrumental, 'instrumental');
#
def Energy(y_data, y_name):
    plt.scatter(dataset.energy, y_data, c=KM2.labels_, cmap=matplotlib.colors.ListedColormap(colors), s=75)
    plt.xlabel('energy')
    plt.ylabel(y_name)
    plt.show()
Energy(dataset.dance, 'dance');
Energy(dataset.tempo, 'tempo');
Energy(dataset.popularity, 'popularity');
Energy(dataset.valence, 'valence');
...
def Dance(y_data, y_name):
    ...
    plt.ylabel(y_name)
    plt.show()
Dance(dataset.dance, 'energy');
Dance(dataset.tempo, 'tempo');
Dance(dataset.popularity, 'popularity');
Dance(dataset.valence, 'valence');
